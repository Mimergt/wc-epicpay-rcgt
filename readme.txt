...
* En el callback se prioriza siempre `checkout_id` retornado por Recurrente cuando difiere del pendiente local.
* Se amplía la ventana de polling para confirmación de tokenización (hasta 45 intentos con pausa de 1 segundo).
* En fallos finales se limpian metadatos/caché pendientes para evitar arrastre de estado entre intentos.

= 2.1.6 =
* Fix adicional para guardado de tarjeta: cache de tokenización desde más eventos webhook (no solo `setup_intent.succeeded`) con extracción flexible de `payment_method_id` y `payment_intent_id`.
* Fallback de retorno: si Recurrente devuelve `payment_method_id` o `checkout_id` en query string, ahora se aprovecha para confirmar y guardar la tarjeta.
* Logging de diagnóstico en callback de tokenización (status y llaves recibidas en query) para depuración precisa.
* Se mantiene polling ampliado del checkout para tolerar demoras de propagación.

= 2.1.5 =
* Fix de confirmación de tarjeta guardada: normalización de respuestas `GET /api/checkouts/{id}` cuando el checkout viene envuelto en `checkout` o `data`.
* Se amplía el polling de tokenización para tolerar mayor latencia de propagación del `payment_method_id`.
* Se agregan fallbacks de extracción para `payment_method` en formato string y para `setup_intent.payment_method_id` / `payment_intent.payment_method_id` / `payment.payment_method_id`.
* Nuevo log informativo de recepción de webhook con el evento detectado para diagnóstico en producción.
...
* Retry automático al consultar `GET /api/checkouts/{id}` para esperar propagación del método de pago.
* Limpieza de notices para evitar mensajes contradictorios de éxito/error en "Métodos de pago".
